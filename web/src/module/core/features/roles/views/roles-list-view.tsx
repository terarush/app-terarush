...
import { useRoleDialog } from '../hooks/use-role-dialog';
import { invalidateRolesCache } from '../hooks/use-roles';
...
export function RolesListView() {
  ...
  const canCreate = can(PERM.roles.create);
  ...
  const TABLE_HEAD = useMemo(
    () => [
      ...
      { id: 'permissions', label: t('table.permissions') },
  ...
  const listParams = useMemo(
    () => ({ page: table.page + 1, limit: table.rowsPerPage, search, include_global: true }),
    [table.page, table.rowsPerPage, search]
  );

  const { data, meta, loading, error, refresh } = useRoleList(listParams);

  const selectedRole = useMemo(
    () => (dialog.id ? (data.find((r) => r.id === dialog.id) ?? null) : null),
    [dialog.id, data]
  );
  ...
  return (
    <DashboardContent maxWidth="xl">
      ...
      <RoleDetailDialog
        ...
        role={dialog.mode === 'view' ? selectedRole : null}
      ...
      <RoleFormDialog
        ...
        seed={dialog.mode === 'edit' ? selectedRole : null}
        onClose={dialog.close}
        onSaved={handleSaved}
      />
