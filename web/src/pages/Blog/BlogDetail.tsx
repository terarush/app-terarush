		...
		return (
			<div className="min-h-screen bg-background flex flex-col">
				...
				<div className="flex-1 flex flex-col items-center justify-center pt-24 space-y-4">
					...
					<p className="text-muted-foreground animate-pulse font-medium">Preparing your article...</p>
				</div>
				<Footer />
			</div>
		);
			...
			<div className="min-h-screen bg-background flex flex-col">
				...
				<main className="flex-1 pt-32">
					<div className="container mx-auto px-4 text-center">
						...
						<h1 className="text-3xl font-bold mb-4">{error || "Blog Post Not Found"}</h1>
						<p className="text-muted-foreground mb-8 max-w-md mx-auto">
							The article you're looking for might have been moved or deleted.
						</p>
						<Button onClick={() => navigate("/blog")} className="rounded-xl px-8 py-6">
						...
						<div className="container mx-auto px-4 pb-12 sm:pb-20 md:pb-32">
							...
							>
								...
								<nav className="hidden sm:flex items-center gap-2 text-white/70 text-xs sm:text-sm font-medium mb-4 sm:mb-6">
									...
									<ChevronRight className="h-3 w-3 sm:h-4 sm:w-4" />
									<span className="text-white truncate max-w-[200px]">{blog.category || 'Article'}</span>
								</nav>
								...
								<div className="hidden sm:flex flex-wrap items-center gap-6 sm:gap-8 text-white/90">
									...
									<div>
										<p className="text-xs sm:text-sm font-bold">{blog.author}</p>
										<p className="text-xs text-white/60 line-clamp-1">{blog.user?.bio ? blog.user.bio.split('\n')[0] : 'Contributor'}</p>
									</div>
									</div>
									<div className="flex items-center gap-4 sm:gap-6">
										...
										<div className="flex items-center gap-2">
											...
											<span className="text-xs sm:text-sm font-medium">{blog.view_count} views</span>
										</div>
									</div>
								</div>
							</motion.div>
					...
					<div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-12">
							...
							<article className="bg-card border border-border/40 rounded-lg sm:rounded-xl p-6 sm:p-8 md:p-16 shadow-xl shadow-primary/5">
								...
								{blog.tags && (
									<div className="mt-12 sm:mt-16 pt-8 sm:pt-10 border-t border-border/40">
										<div className="flex items-center gap-3 mb-4 sm:mb-6">
											...
											<h3 className="text-base sm:text-lg font-bold">Related Topics</h3>
										</div>
										<div className="flex flex-wrap gap-2">
											{blog.tags.split(",").map((tag, i) => (
												<span key={i} className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg sm:rounded-xl bg-muted/50 border border-border/50 text-xs sm:text-sm font-medium hover:bg-primary/10 hover:border-primary/30 cursor-pointer transition-all">
													#{tag.trim()}
												</span>
											))}
										</div>
								...
								<div className="mt-12 sm:mt-16 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 py-6 sm:py-8 border-t border-border/40">
									<div className="w-full sm:w-auto">
										<span className="text-xs font-bold text-muted-foreground uppercase tracking-widest block mb-3 sm:mb-0">Share Article</span>
										<div className="flex gap-2">
											<Button variant="outline" size="icon" className="rounded-lg sm:rounded-xl h-9 sm:h-10 w-9 sm:w-10">
												<Twitter className="h-3.5 sm:h-4 w-3.5 sm:w-4" />
											</Button>
											<Button variant="outline" size="icon" className="rounded-lg sm:rounded-xl h-9 sm:h-10 w-9 sm:w-10">
												<Facebook className="h-3.5 sm:h-4 w-3.5 sm:w-4" />
											</Button>
											<Button variant="outline" size="icon" className="rounded-lg sm:rounded-xl h-9 sm:h-10 w-9 sm:w-10" onClick={copyToClipboard}>
												<LinkIcon className="h-3.5 sm:h-4 w-3.5 sm:w-4" />
											</Button>
										</div>
									</div>
									<div className="w-full sm:w-auto flex items-center gap-2 sm:gap-4">
										<Button variant="ghost" className="rounded-lg sm:rounded-xl gap-2 text-xs sm:text-sm flex-1 sm:flex-none">
						...
						<aside className="lg:col-span-3 space-y-6 sm:space-y-8">
							<div className="p-6 sm:p-8 rounded-lg sm:rounded-xl bg-card border border-border/40 shadow-lg">
								...
								<p className="text-xs sm:text-sm text-muted-foreground mb-4 sm:mb-6">Get the latest articles and insights delivered to your inbox.</p>
								<div className="space-y-2 sm:space-y-3">
									...
									/>
									<Button className="w-full rounded-lg sm:rounded-xl py-5 sm:py-6 font-bold text-xs sm:text-sm">Subscribe</Button>
								</div>
							</div>

							{headings.length > 0 && (
								<div className="space-y-4 sm:space-y-6 p-6 sm:p-8 rounded-lg sm:rounded-xl bg-card border border-border/40">
									<h3 className="text-lg sm:text-xl font-bold px-0 sm:px-2">Table of Contents</h3>
